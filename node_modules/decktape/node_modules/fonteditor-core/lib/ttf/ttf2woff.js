...
function ttf2woff(ttfBuffer) {
  ...
  for (i = 0; i < numTables; ++i) {
    ...
    var sfntData = ttfReader.readBytes(tableEntry.offset, tableEntry.length); // 对数据进行压缩

    if (options.deflate) {
      deflatedData = options.deflate(sfntData); // 这里需要判断是否压缩后数据小于原始数据
  ...
  for (i = 0, l = tableEntries.length; i < l; ++i) {
    ...
    tableEntry.offset = woffSize; // 4字节对齐
  ...
  if (options.metadata) {
    ...
    woffHeader.metaOffset = woffSize; // metadata header + length
  ...
  woffHeader.totalSfntSize = ttfSize; // 写woff数据

  var woffWriter = new _writer["default"](new ArrayBuffer(woffSize)); // 写woff头部
  ...
  woffWriter.writeUint32(woffHeader.privLength); // 写woff表索引
