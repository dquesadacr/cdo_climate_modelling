...
function getArcCenter(x1, y1, x2, y2, fa, fs, rx, ry, sin_φ, cos_φ) {
  ...
  var cy = sin_φ * cxp + cos_φ * cyp + (y1 + y2) / 2; // Step 4.
  //
...
function a2c(x1, y1, x2, y2, fa, fs, rx, ry, φ) {
  ...
  var cos_φ = Math.cos(φ * TAU / 360); // Make sure radii are valid
  //
  ...
  rx = radii[0];
  ry = radii[1]; // Get center parameters (cx, cy, θ1, Δθ)
  //

  var cc = getArcCenter(x1, y1, x2, y2, fa, fs, rx, ry, sin_φ, cos_φ);
  var result = [];
  var θ1 = cc[2];
  var Δθ = cc[3]; // Split an arc to multiple segments, so each segment
  ...
  return result.map(function (curve) {
    for (var _i = 0; _i < curve.length; _i += 2) {
      ...
      var yp = sin_φ * x + cos_φ * y; // translate
